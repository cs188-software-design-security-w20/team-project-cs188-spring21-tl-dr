...
describe('/textss', function(){
    ...
    describe('Find', function(){
        ...
        //     agent
        //     .get('/textss/'+oneId2+'?populate=toPop')
        //     .set('x-tag',tag)
        //     .then(function(resp){
        ...
        it('should load next page for pagination', function(done){
            agent
            .get('/textss?lastId='+last+'')
            .set('x-tag',tag)
            .expect(200,done);
        });
        it('should filter by date range', function(done){
            agent
            .get('/textss?from='+from+'&to='+new Date().toISOString())
            .set('x-tag',tag)
            .then(function(resp){
                resp.body.data.length.should.be.above(0);
                done();
