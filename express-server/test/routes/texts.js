...
describe('/texts', function(){
    ...
    describe('Find', function(){
        ...
        //     agent
        //     .get('/texts/'+oneId2+'?populate=toPop')
        //     .set('x-tag',tag)
        //     .then(function(resp){
        ...
        it('should load next page for pagination', function(done){
            agent
            .get('/texts?lastId='+last+'')
            .set('x-tag',tag)
            .expect(200,done);
        });
        it('should filter by date range', function(done){
            agent
            .get('/texts?from='+from+'&to='+new Date().toISOString())
            .set('x-tag',tag)
            .then(function(resp){
                resp.body.data.length.should.be.above(0);
                done();
